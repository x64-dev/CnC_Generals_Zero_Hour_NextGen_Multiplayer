...
void Drawable::drawHealing(const IRegion2D* healthBarRegion)
{
	...
	if( obj->isKindOf( KINDOF_NO_HEAL_ICON ) || BitTestEA( obj->getStatusBits(), OBJECT_STATUS_SOLD ))
		return;
...
void Drawable::drawConstructPercent( const IRegion2D *healthBarRegion )
{
	...
	if( obj == NULL || BitTestEA( obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == FALSE ||
			BitTestEA( obj->getStatusBits(), OBJECT_STATUS_SOLD ) == TRUE )
	{
...
void Drawable::drawHealthBar(const IRegion2D* healthBarRegion)
{
			...
			(isSelected() || (TheInGameUI && (TheInGameUI->getMousedOverDrawableID() == getID()))) )
	{
		...
		if( BitTestEA( obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) || (obj->isDisabled() && !obj->isDisabledByType(DISABLED_HELD)) )
		{
